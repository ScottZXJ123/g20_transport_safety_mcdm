...
# Step 2.2 Normalization 2 (Min-Max)
normalized_decision_matrix_2 = np.zeros_like(decision_matrix, dtype=float)
for j in range(n):
    if j in negative_indicators:
        normalized_decision_matrix_2[:, j] = (np.max(decision_matrix[:, j]) - decision_matrix[:, j]) / (np.max(decision_matrix[:, j]) - np.min(decision_matrix[:, j]))
    else:
        normalized_decision_matrix_2[:, j] = (decision_matrix[:, j] - np.min(decision_matrix[:, j])) / (np.max(decision_matrix[:, j]) - np.min(decision_matrix[:, j]))
