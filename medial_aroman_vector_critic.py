...
# Calculate weights using the CRITIC method
data = df.to_numpy()
m, n = data.shape

# Normalize the decision matrix using the formulas from the image
...
# Calculate the correlation coefficients between criteria
correlations = np.corrcoef(normalized_data.T)

# Calculate the amount of information (cj) for each criterion
cj = std_dev * np.sum(1 - correlations, axis=1)

# Calculate the weights using the CRITIC method
weights = cj / np.sum(cj)
