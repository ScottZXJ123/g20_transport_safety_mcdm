...
p_ij = np.where(data.sum(axis=0) == 0, 0, data / data.sum(axis=0))

# Initialize the entropy with zeros
e_j = np.zeros(n)

# Calculate the entropy e_j for each criterion
k = 1 / np.log(m)
for j in range(n):
    p_ij_non_zero = p_ij[:, j] > 0
    e_j[j] = -k * np.sum(p_ij[p_ij_non_zero, j] * np.log(p_ij[p_ij_non_zero, j]))

# Compute the initial weights w_j from the entropy values
w_j = (1 - e_j) / (1 - e_j).sum()

# Assume alpha_j is equal for all criteria for this example
alpha_j = np.ones(n)

# Calculate the normalized weights beta_j
weights = alpha_j * w_j / (alpha_j * w_j).sum()
